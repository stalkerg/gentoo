...
"
MY_PV=$(ver_rs 3 '-')
SRC_URI="https://codeberg.org/redict/redict/archive/${MY_PV}.tar.gz"
S="${WORKDIR}/redict"
...
src_install() {
	...
	newconfd "${FILESDIR}/redict.confd-r2" redict
	newinitd "${FILESDIR}/redict.initd-6" redict

	systemd_newunit "${FILESDIR}/redict.service-4" redict.service
	newtmpfiles "${FILESDIR}/redict.tmpfiles-2" redict.conf

	newconfd "${FILESDIR}/redict-sentinel.confd-r1" redict-sentinel
	newinitd "${FILESDIR}/redict-sentinel.initd-r1" redict-sentinel

	insinto /etc/logrotate.d/
	newins "${FILESDIR}/${PN}.logrotate" ${PN}

	# 00-RELEASENOTES BUGS MANIFESTO
	...
	dosym redict-server /usr/sbin/redredictis-sentinel
